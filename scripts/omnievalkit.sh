...
# Instead of hardcoding a port, we can let torchrun find an open one by using 0

CMD="torchrun --nproc_per_node=${GPUS_PER_NODE} --rdzv-backend=c10d --rdzv-endpoint=localhost:0 eval_main.py ${OPT_ARGS}"
